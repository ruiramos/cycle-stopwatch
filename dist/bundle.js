...
/***/ function(module, exports, __webpack_require__) {
	...
	var _setPlaying = function _setPlaying(p) {
	  document.querySelector('iframe').contentWindow.postMessage({ playing: p }, '*');
	};
	
	// returns an array with `n` values, set to `undefined`
	var arrayWithElm = function arrayWithElm(n) {
	...
	var pad = function pad(n, s) {
	  ...
	  var str = String(s),
	      pad = arrayWithElm(n - str.length).map(function () {
	    return v;
	  });
	...
	var pad3 = function pad3(s) {
	  return pad(3, s);
	};
	var pad2 = function pad2() {
	...
	var formatMinutes = function formatMinutes(ms) {
	  return pad2(Math.floor(ms / 1000 / 60));
	};
	var formatSeconds = function formatSeconds(ms) {
	  return pad2(Math.floor(ms / 1000 % 60));
	};
	var formatMiliseconds = function formatMiliseconds(ms) {
	  return pad3(ms % 1000);
	};
	
	var toggleTimerText = function toggleTimerText(isTimerOn) {
	...
	function main(drivers) {
	  var STEP = 32; // timer goes every 32 ms
	
	  var pauser = new _rx.Subject();
	
	  var timer$ = _rx.Observable.timer(0, STEP).map(function (c, i) {
	    return STEP;
	  }).pausable(pauser);
	
	  var toggleButton$ = drivers.DOM.select('button[data-hook=toggle-timer]').events('click').map(function () {
	    return ':toggle';
	  });
	
	  var resetButton$ = drivers.DOM.select('button[data-hook=reset-timer]').events('click').map(function () {
	    return ':reset';
	  });
	
	  var timerStatus$ = toggleButton$.scan(function (p, v) {
	    if (v === ':toggle') {
	      pauser.onNext(!p);
	      _setPlaying(!p);
	      return !p;
	  ...
	  var timerValue$ = _rx.Observable.merge(timer$, resetButton$).scan(function (acc, x) {
	  ...
	  var $events = _rx.Observable.combineLatest(timerValue$, timerStatus$);
	
	  var $actions = $events.scan(function (a, x) {
	    return x;
	  });
	
	  return {
	    DOM: $actions.map(function (_ref) {
	      var _ref2 = _slicedToArray(_ref, 2);
	
	      var msTimerValue = _ref2[0];
	      var isTimerOn = _ref2[1];
	      return (0, _dom.div)({
	        id: 'timer'
	      }, [(0, _dom.p)({ className: 'title' + (isTimerOn ? ' title--on' : '') }, '⏱'), (0, _dom.div)({ 'className': 'numbers', 'data-hook': 'timer' }, [(0, _dom.span)({ 'className': 'numbers__minutes' }, formatMinutes(msTimerValue)), (0, _dom.span)({ className: 'numbers__separator' }, ':'), (0, _dom.span)({ 'className': 'numbers__second' }, formatSeconds(msTimerValue)), (0, _dom.span)({ className: 'numbers__separator' }, ':'), (0, _dom.span)({ 'className': 'numbers__miliseconds' }, formatMiliseconds(msTimerValue))]), (0, _dom.button)({
	        type: 'button',
	        className: 'button--grey',
	        attributes: {
	          'data-hook': 'reset-timer'
	        }
	      }, 'Reset timer'), (0, _dom.button)({
	        type: 'button',
